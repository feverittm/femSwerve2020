...
public class HopperSubsystem extends SubsystemBase {
  ...
  /**
   * Get the position of the belts
   * @return position
   */
  public double getHopperPosition() {
    return mMotor1.getSelectedSensorPosition(0);
  }

  /**
   * Move the hopper incrementally:
   * Positive (+1) moves the balls, belts upwards (toward shooter).
   * Negative (-1) moves the balls back down toward the intake.
   * @param count the number of ball widths to move the hopper belts
   */
  public void shiftHopper(int count) {
    double targetPositionRotations = count * 10.0 * 4096;
    mMotor1.set(ControlMode.Position, targetPositionRotations);
  }
   ...
   * True = Ball Present, False = Ball not there.
   * 
   * @return boolean
   */
